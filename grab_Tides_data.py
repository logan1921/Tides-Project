...
#load up gui
text = "Please select if you want to download Water Level Observations or Predictions."
title = "DFO Station Download"
#load gui for checkbox
wlo = 0
wlp = 0
answer = easygui.choicebox(text, title, ["Water Level Observations","Water level Predictions"])
if answer =="Water Level Observations":
    wlo=1
elif answer =="Water level Predictions":
    wlp=1
print(wlo)    
...
for year in range(int(output[1]),int(output[2])+1):
    for month in range(1,13):
        ...
        if wlo ==1:
            ...
            response = requests.get(f'https://api-iwls.dfo-mpo.gc.ca/api/v1/stations/{stationID}/data?time-series-code=wlp&from={year:04}-{month:02}-01T00%3A00%3A00Z&to={year:04}-{month:02}-01T12%3A00%3A00Z')
...
df = pd.concat(frames)
if wlp==1:
    df.to_csv(f'{stationID}-wlp-{output[1]}-{output[2]}.csv', index=False) 
if wlo==1:
    df.to_csv(f'{stationID}-wlo-{output[1]}-{output[2]}.csv', index=False) 
